...
async function isType(fsStatType, statsMethodName, filePath) {
	...
	try {
		const stats = await promisify(fs$1[fsStatType])(filePath);
...
function toCSS(object) {
    return stringify_2(object, (value, indent, stringify) => {
        if (typeof value === 'string') {
            return value;
        }
...
(async () => {
    const cliOpts = commander.exports.program.opts(), cosmic = cliOpts?.config
        ...
        return [key, val];
    }), output = formatModule(Object.keys(cssMap).reduce((obj, cur) => ({ ...obj, ...cssMap[cur] }), {}));
    require$$2__default["default"].writeFileSync(require$$0__default["default"].resolve(process.cwd(), outputPath), `:root ${toCSS(output)}`);
})();
